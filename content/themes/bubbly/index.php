<?php get_header(); ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('item-list mbottom'); ?>>
        ...
        <h2><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
            ...
            <div class="postmeta">
       		    <p class="vsmall pnone"><span><?php _e('By', 'bubbly'); ?>  <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) )?>" title="<?php sprintf( esc_attr__( 'View all posts by %s', 'bubbly' ), get_the_author() ) ?>"><?php echo get_the_author() ?> </a>
     		       <?php _e('on', 'bubbly'); ?> <span class="mdate"><?php echo the_time(get_option( 'date_format' )) ?></span></span></p>
			</div>
        </div>
